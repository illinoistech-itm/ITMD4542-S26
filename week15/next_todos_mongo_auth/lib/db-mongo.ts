...
type TodoDoc = { userId: string; title: string; completed: boolean };
...
function toTodo(doc: WithId<TodoDoc>): Todo {
  return { id: doc._id.toString(), userId: doc.userId, title: doc.title, completed: doc.completed };
}
...
export const db = {
  ...
  async createTodo(title: string, userId: string): Promise<Todo> {
    const col = await getCollection;
    const result = await col.insertOne({ userId, title, completed: false });
    return { id: result.insertedId.toString(), userId, title, completed: false };
  },
